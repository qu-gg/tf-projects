...
w1 = tf.Variable(tf.random_normal([4, k], stddev=(B*(1/4))**2), name="Weights1")
b1 = tf.Variable(tf.zeros([k]), name="Bias1")
w2 = tf.Variable(tf.random_normal([k, l], stddev=(B*(1/k))**2), name="Weights2")
b2 = tf.Variable(tf.zeros([l]), name="Bias2")
w3 = tf.Variable(tf.random_normal([l, m], stddev=(B*(1/l))**2), name="Weights3")
b3 = tf.Variable(tf.zeros([m]), name="Bias3")
w4 = tf.Variable(tf.random_normal([m, n], stddev=(B*(1/m))**2), name="Weights4")
b4 = tf.Variable(tf.zeros([n]), name="Bias4")
w5 = tf.Variable(tf.random_normal([n, 3], stddev=(B*(1/n))**2), name="Weights5")
